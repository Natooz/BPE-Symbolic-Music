...
class TSDPlusPVDm(MIDITokenizer):
    ...
    def track_to_tokens(self, track: Instrument) -> List[Event]:
        ...
        for n, note in enumerate(track.notes):
            ...
            events.append(Event(type='PitchVelDur', time=note.start,
            ...
            if token.split("_")[0] == "TimeShift":
                ...
                try:
                    if tokens[ti - 1].split("_")[0] == "Program":
                        pitch, vel, dur = token.split("_")[1].split('-')
    ...
    def _create_token_types_graph(self) -> Dict[str, List[str]]:
        ...
        dic["PitchVelDur"] = ["Program", "TimeShift"]
        dic["TimeShift"] = ["Program"]

        if self.additional_tokens["Chord"]:
            dic["Chord"] = ["PitchVelDur"]
            dic["TimeShift"] += ["Program"]
