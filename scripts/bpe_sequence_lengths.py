...
def variation(n1, n2) -> float:
    return (n2 - n1) / n1 * 100


if __name__ == "__main__":
    set_seed(777)
    (out_dir := Path("analysis", "bpe_seq_len_variations")).mkdir(
    ...
    labels = (
        ["noBPE_tiny", "noBPE"]
        + [f"bpe{f}" for f in BPE_VOCAB_SIZES]
        + ["PVm", "PVDm", "CPWord", "Octuple"]
    )

    def get_seq_lengths_subset(
        midi_path: Path, tokenizer: MIDITokenizer
        ...
        for file_path in midi_paths:
            ...
            tokens = tokenizer.midi_to_tokens(
                midi
            )  # We retokenize here as json files did not keep the names of mid
            if len(tokens) == 0:
                continue
            tok_lengths.append(len(tokens[0]))
            tokens_per_beats.append(
                tok_lengths[-1] / (midi.max_tick / midi.ticks_per_beat)
            )
        return tok_lengths, tokens_per_beats

    all_var_lens, all_var_tpb = [[] for _ in range(len(labels))], [
    ...
    for ei, exp in enumerate(experiments):
        ...
        for bi, baseline in enumerate(exp.baselines):
            lengths, tpb = get_seq_lengths_subset(
                exp.data_path_midi, baseline.tokenizer
            )
            mean_length, mean_tpb = mean(lengths), mean(tpb)
            if bi == 0:
                original_mean_len, original_mean_tpb = mean_length, mean_tpb
            vocab_size = (
                prod([len(v) for v in baseline.tokenizer.vocab])
                if isinstance(baseline.tokenizer.vocab, list)
                else len(baseline.tokenizer)
            )
            var_seq_len, var_tpb = variation(original_mean_len, mean_length), variation(
                original_mean_tpb, mean_tpb
            )
            all_var_lens[bi].append(float(var_seq_len))
            all_var_tpb[bi].append(float(var_tpb))

            logger.debug(
                f"{baseline.name} : {mean_tpb:.2f} ± {std(tpb):.2f} tokens / beat"
    ...
    for type_, seq in [
        ("var seq len", all_var_lens),
        ("var tokens / beat", all_var_tpb),
    ]:
        logger.debug(f"\nMean {type_}:")
