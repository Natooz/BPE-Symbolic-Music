...
if __name__ == '__main__':
    midi_paths = list(Path('../data', 'POP909').glob('**/*.mid'))
    merged_out_dir = Path('../data', 'POP909-merged')
    merged_out_dir.mkdir(parents=True, exist_ok=True)

    for i, midi_path in enumerate(tqdm(midi_paths, desc='PRE-PROCESSING POP909 DATASET')):
